...
document.getElementById("immediate-start").addEventListener("click", (e) => {
  e.preventDefault();
  if (abortController) {
...
document.getElementById("delayed-start").addEventListener("click", (e) => {
  e.preventDefault();
  if (abortController) {
    console.log("aborting request & starting new one in 3 seconds");
    // Abort the request synchronously.
    // This lets us use the user activation from clicking the button on safari to trigger webauthn.
    abortController.abort();
  }
  window.setTimeout(() => startConditionalRequest(), 3000);
});